...
#include "obj_dir_wbtoliconverter/Vwbtoliconverter.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_wbtoliconverter/Vwbtoliconverter__Syms.h"
#include "svdpi.h"
#endif
...
extern "C" {
  ...
  #if VLINETRACE
  void trace( Vwbtoliconverter_t *, char * );
  #endif
...
Vwbtoliconverter_t * create_model( const char *vcd_filename ) {

  Vwbtoliconverter_t * m;
  Vwbtoliconverter   * model;

  Verilated::randReset( 0 );
  Verilated::randSeed( 0 );

  m     = (Vwbtoliconverter_t *) malloc( sizeof(Vwbtoliconverter_t) );
  model = new Vwbtoliconverter();
  ...
  m->wb_clk_i = &model->wb_clk_i;
  m->wb_rst_i = &model->wb_rst_i;
  m->wbs_ack_o = &model->wbs_ack_o;
  m->wbs_adr_i = &model->wbs_adr_i;
  m->wbs_cyc_i = &model->wbs_cyc_i;
  m->wbs_dat_i = &model->wbs_dat_i;
  m->wbs_dat_o = &model->wbs_dat_o;
  m->wbs_sel_i = &model->wbs_sel_i;
  m->wbs_stb_i = &model->wbs_stb_i;
  m->wbs_we_i = &model->wbs_we_i;
...
void destroy_model( Vwbtoliconverter_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  Vwbtoliconverter * model = (Vwbtoliconverter *) m->model;
...
void comb_eval( Vwbtoliconverter_t * m ) {
...
void seq_eval( Vwbtoliconverter_t * m ) {
...
#if VLINETRACE
void trace( Vwbtoliconverter_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_wbtoliconverter__v );
